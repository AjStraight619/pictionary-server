...
type Game struct {
	mu              sync.Mutex // NOTE: Maybe use rwLock here??
	Id              string
	Hub             *ws.Hub
	Players         []*Player
...
func (g *Game) StartGame(onRoundComplete func()) error {
	...
	g.Round.StartTimer(roundDuration, func() {
		log.Printf("Round %d ended!", g.Round.Count)
		onRoundComplete()
	})
...
func (g *Game) AdvanceToNextDrawer(onRoundComplete func()) (*Player, error) {
	...
	if g.Round.IsActive {
		...
		g.Round.PlayersDrawnThisRound[currentDrawer.Id] = struct{}{}
	}

	// Check if all players have drawn
	if len(g.Round.PlayersDrawnThisRound) == totalPlayers {
		// End the current round
		g.Round.NextRound()
		g.Round.PlayersDrawnThisRound = make(map[string]struct{}) // Clear for the next round
